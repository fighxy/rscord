...
export function NotificationSettings({ onSettingsChange }: NotificationSettingsProps) {
  ...
  useEffect(() => {
    ...
    const savedSettings = localStorage.getItem('rscord-notification-settings');
    if (savedSettings) {
      try {
  ...
  useEffect(() => {
    ...
    localStorage.setItem('rscord-notification-settings', JSON.stringify(settings));
  ...
  const sendTestNotification = () => {
    ...
    if (settings.desktop) {
      new Notification('RSCord - Тестовое уведомление', {
            ...
            <div className="flex items-start gap-4">
              ...
              <div className="flex-1">
                ...
                <p className="text-sm text-text-secondary mb-4 leading-relaxed">
                  Для получения уведомлений от RSCord необходимо предоставить соответствующие разрешения в браузере.
