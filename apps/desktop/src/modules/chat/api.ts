...
export async function listMessages(channel_id: string) {
  const res = await httpClient.get<Message[]>(`/channels/${channel_id}/messages`);
  return res.data;
}

export async function sendMessage(channel_id: string, author_id: string, content: string) {
  const res = await httpClient.post<{ message: Message }>(`/channels/${channel_id}/messages`, { author_id, content });
